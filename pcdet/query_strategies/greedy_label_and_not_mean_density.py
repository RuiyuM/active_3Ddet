...
class Double_greedy_not_mean(Strategy):
    def __init__(self, model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg):

        super(Double_greedy_not_mean, self).__init__(model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg)
    ...
    def query(self, leave_pbar=True, cur_epoch=None):
        ...
        # 试试400-> 200如何
        #total_num_cyclist
        # 和原版的改动是每次200，然后： total_num_pedestrain 400 -》500
        ...
        global_density_low = [int(sorted_density[unique_label][-int(self.alpha * label_counts[unique_label])]) for
                              unique_label in range(len(unique_labels))]
        x_axis = [np.linspace(-50, int(global_density_max[i]) + 50, 1000) for i in range(len(global_density_max))]
        uniform_dist_per_cls = [
            uniform.pdf(x_axis[i], global_density_low[i], global_density_high[i] - global_density_low[i]) for i in
            range(len(global_density_max))]
