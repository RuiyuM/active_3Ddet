...
class Facility_location_700_uniform_300(Strategy):
    def __init__(self, model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg):

        super(Facility_location_700_uniform_300, self).__init__(model, labelled_loader, unlabelled_loader, rank,
    ...
    def query(self, leave_pbar=True, cur_epoch=None):
        ...
        selector = FacilityLocationSelection(1200, 'euclidean')
        ...
        while total_num_pedestrain <= 4000 and len(selected_frames) <= 400:
            if index_of_greedy == 0:  # initially, we randomly select a frame.
                ...
                if best_frame is not None:
                    total_num_car += new_predicted_label_dict[best_frame]['Car'] if new_predicted_label_dict[best_frame]['Car'] is not None else 0
                    total_num_pedestrain += new_predicted_label_dict[best_frame]['Pedestrian'] if new_predicted_label_dict[best_frame]['Pedestrian'] is not None else 0
                    total_num_cyclist += new_predicted_label_dict[best_frame]['Cyclist'] if new_predicted_label_dict[best_frame]['Cyclist'] is not None else 0
        ...
        selected_frames = self.sorting_out_excessive_car_samples(selected_frames, predicted_label_dict_deep_copy, confidence_list)
    ...
    def calculate_closeness(self, sample, confidence_list):
        # Assuming 'Car', 'Cyclist', 'Pedestrian' are keys with tensor values
        total_num = sample['Car'] + sample['Pedestrian'] + sample['Cyclist']

        current_difference = abs(
            sample['Car'] / total_num * confidence_list[0] - sample['Pedestrian'] / total_num * confidence_list[
                1]) + abs(
            sample['Car'] / total_num * confidence_list[0] - sample['Cyclist'] / total_num * confidence_list[2]) + abs(
            sample['Pedestrian'] / total_num * confidence_list[1] - sample['Cyclist'] / total_num * confidence_list[2])
