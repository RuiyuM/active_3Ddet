...
class Double_greedy_5_1_1(Strategy):
    def __init__(self, model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg):

        super(Double_greedy_5_1_1, self).__init__(model, labelled_loader, unlabelled_loader, rank, active_label_dir,
                                                     cfg)
    ...
    def query(self, leave_pbar=True, cur_epoch=None):
        ...
        index_of_greedy = 0
        # 试试400如何
        # 200 query version pedestrain = 500
        while total_num_pedestrain <= 500 and index_of_greedy <= 10000:
            if index_of_greedy == 0:  # initially, we randomly select a frame.
                ...
                best_frame = None
                for key, value in true_label.items():
                    scaled_car = (total_num_car + value[
                        'Car']) * 1  # Scale factor for cars is 1 since they are the baseline
                    scaled_pedestrian = (total_num_pedestrain + value[
                        'Pedestrian']) * 5  # Scale to match cars, 2 pedestrians are equivalent to 10 cars
                    scaled_cyclist = (total_num_cyclist + value[
                        'Cyclist']) * 5  # Scale to match cars, 2 cyclists are equivalent to 10 cars

                    # Calculate difference based on scaled values
                    current_difference = abs(scaled_car - scaled_pedestrian) + abs(scaled_car - scaled_cyclist) + abs(
                        scaled_pedestrian - scaled_cyclist)
                ...
                if best_frame is not None:
                    ...
                    total_num_pedestrain += true_label[best_frame]['Pedestrian'] if true_label[best_frame][
                                                                                        'Pedestrian'] is not None else 0
                    total_num_cyclist += true_label[best_frame]['Cyclist'] if true_label[best_frame][
                                                                                  'Cyclist'] is not None else 0
        ...
        global_density_high = [int(sorted_density[unique_label][int(self.alpha * label_counts[unique_label])]) for
                               unique_label in range(len(unique_labels))]
        global_density_low = [int(sorted_density[unique_label][-int(self.alpha * label_counts[unique_label])]) for
                              unique_label in range(len(unique_labels))]
        x_axis = [np.linspace(-50, int(global_density_max[i]) + 50, 1000) for i in range(num_class)]
        ...
        for j in range(self.cfg.ACTIVE_TRAIN.SELECT_NUMS * 2):
