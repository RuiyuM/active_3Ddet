...
class Cls_Regression_CRBSampling(Strategy):
    def __init__(self, model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg):

        super(Cls_Regression_CRBSampling, self).__init__(model, labelled_loader, unlabelled_loader, rank, active_label_dir, cfg)
    ...
    def adding_weight_to_unique_proportions(self, pred_dicts, batch_inx):
        pred_logits = pred_dicts[batch_inx]['pred_logits']
        mean_logits = pred_logits.mean(dim=0)
        # To keep the result as (1, 3) instead of (3,), you can use unsqueeze
        # mean_logits = mean_logits.unsqueeze(0)
        mean_logits = F.softmax(mean_logits, dim=0)
        return mean_logits
